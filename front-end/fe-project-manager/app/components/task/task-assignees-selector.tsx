...
const TaskAssigneesSelector = ({
  ...
  task: Task;
  assignees: User[];
  projectMembers: { user: User; role: 'admin' | 'manager' | 'contributor' | 'viewer' }[];
}) => {
  ...
  return (
    <div className='mb-6'>
      ...
      <div className='flex flex-wrap gap-2 mb-2'>
        {selectedIds.length === 0 ? (
          ...
          projectMembers
            ...
            .map((m) => (
              <div key={m.user._id} className='flex items-center bg-blue-100 rounded px-2 py-1'>
                <Avatar>
      ...
      <div className='relative'>
        ...
        >
          {selectedIds.length === 0 ? 'Selected assignees' : `${selectedIds.length} selected`}
        </button>

        {dropdownOpen && (
          <div className='absolute z-10 mt-1 w-full bg-white border rounded shadow-lg max-h-60 overflow-y-auto'>
            <div className='flex justify-between px-2 py-1 border-b'>
              <button className='text-xs text-blue-600' onClick={hanldeSelectAll}>
                Select all
              </button>
